...
%% Dynamical matrices D of the last semi-infinite layer (eq. 3.5 and 3.6)
...
%% Characteristic matrices M of the combined structure (eq. 3.8)
M_s_11 = D_1_inv_s_11;
M_s_12 = D_1_inv_s_12;
M_s_21 = D_1_inv_s_21;
M_s_22 = D_1_inv_s_22;
...
%% Reflection intensity coefficients R (eq. 3.14)
r_s = M_s_21 ./ M_s_11;
r_p = M_p_21 ./ M_p_11;
...
% theta_1 [rad]: reflection angle in the first, semi-infinite layer;
% d_k [m]: layer thicknesses of layer k;
% epsilon: permitivities of the different layers
% lambda [m]:  light wavelength;

%% the characteristic matrix of the kth layer (eq. 3.1)
q_k_s = (epsilon_k - epsilon_1 .* sin(alpha_1).^2).^0.5;% (eq. 3.3)
q_k_p = (epsilon_k ./ q_k_s);% (eq. 3.4)
beta_k = (2 * pi * d_k) ./ lambda .* q_k_s;% (eq. 3.2)
